...
{
  union {
    net::mutable_buffer head;
    net::mutable_registered_buffer registered{};
  };
  std::span<const net::mutable_buffer> tail;

  std::size_t buffer_count() const {return tail.size() + 1u;}

  mutable_buffer_sequence(net::mutable_registered_buffer buffer = {}) : registered(buffer)
  {
  }

  mutable_buffer_sequence(net::mutable_buffer head)
  {
    this->head = head;
  }
  mutable_buffer_sequence(std::span<const net::mutable_buffer> spn)
  {
    if (!spn.empty())
    {
      head = spn.front();
  ...
  struct const_iterator
  {
    ...
    using pointer           = const net::mutable_buffer*;
    using reference         = const net::mutable_buffer&;

    const_iterator(net::mutable_buffer head,
                   std::span<const net::mutable_buffer> tail,
   ...
   private:
    ...
    net::mutable_buffer head_;
    std::span<const net::mutable_buffer> tail_;
  ...
  {
    return registered.id() != net::registered_buffer_id();
  }
};
...
{
  union {
    net::const_buffer head;
    net::const_registered_buffer registered;
  };
  std::span<const net::const_buffer> tail;

  std::size_t buffer_count() const {return tail.size() + 1u;}

  const_buffer_sequence(net::const_registered_buffer buffer = {}) : registered(buffer)
  {
  }

  const_buffer_sequence(net::const_buffer head)  { this->head = head; }
  const_buffer_sequence(std::span<const net::const_buffer> spn)
  {
    if (!spn.empty())
    {
      head = spn.front();
  ...
  const_buffer_sequence& operator=(net::const_buffer cb)
  {
    head = cb;
    tail = {};
    return *this;
  ...
  const_buffer_sequence& operator=(std::span<const net::const_buffer> spn)
  {
    if (!spn.empty())
    {
      head = spn.front();
  ...
  struct const_iterator
  {
    ...
    using difference_type   = std::ptrdiff_t;
    using pointer           = const net::const_buffer*;
    using reference         = const net::const_buffer&;

    const_iterator(net::const_buffer head,
                   std::span<const net::const_buffer> tail,
                   std::size_t offset = std::numeric_limits<std::size_t>::max()) : head_(head), tail_(tail), offset_(offset) {}
   ...
   private:
    ...
    net::const_buffer head_;
    std::span<const net::const_buffer> tail_;
