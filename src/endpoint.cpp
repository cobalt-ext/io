...
{
  throw_exception(
      cobalt::io::bad_endpoint_access(), loc);
}
...
std::size_t tag_invoke(make_endpoint_tag<AF_UNIX>,
                       ...
                       std::string_view sv)
{
  auto un = new (base) net::detail::sockaddr_un_type{
      .sun_family=AF_UNIX
  };
                       ...
                       std::uint16_t port)
{
  ...
  auto a = net::ip::address_v4::from_string(addr.c_str()).to_uint();
                       ...
                       std::uint16_t port)
{
  ...
  auto a = net::ip::address_v6::from_string(addr.c_str()).to_bytes();
  auto in6 = new (base) net::detail::sockaddr_in6_type{
      .sin6_family=AF_INET6,
      .sin6_port=port
  };
                       ...
                       std::uint16_t port)
{
  ...
  auto ad = net::ip::address::from_string(addr.c_str());
  if (ad.is_v4())
  {
    new (base) net::detail::sockaddr_in4_type{
        .sin_family=AF_INET,
